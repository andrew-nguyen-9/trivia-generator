...
- **Backend/DB**: Supabase (Postgres + auto-REST), RLS public-read
...
| File | Purpose |
|---|---|
| `db/schema.sql` | full schema (facts, questions, daily_sets) — run in Supabase SQL editor |
...
| `frontend/lib/queries.ts` | ALL data access; Supabase with seed-bank fallback |
...
### Database
- writes via service-role key (pipeline only); anon key reads enforced by RLS
- new tables: RLS on + public-read policy; migrations in `db/migrations/` timestamp-prefixed
...
- Do not write to Supabase from the frontend
