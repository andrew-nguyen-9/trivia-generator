...
create table if not exists facts (
  ...
  source        text not null check (source in ('wikipedia','wikipedia_pkg','wikidata','dbpedia','deezer','sleeper','espn','tmdb','restcountries','opentdb','jservice','quizapi','curated','manual')),
...
create table if not exists questions (
  ...
  image_url     text,
  source_url    text,
  audio_url     text,                           -- audio_guess (THE OVERTURE, v3 audio room): streamed Deezer preview
  melody        jsonb,                          -- audio_guess offline synth: [{n,d}] (legacy Jukebox mechanic)
